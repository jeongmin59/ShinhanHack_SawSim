...
public class AnalyzeService {
    ...
    public AnalyzeResponseDto analyzeGet(AnalyzeRequestDto analyzeRequestDto, String userNumber, Long planId) {
        ...
        // 1.
        // 유저 번호를 통해서 계좌 번화를 알아내서 신한 거래내역 조회 API로 거래 내역 받아오기
        ShinhanTransactionRequestDto shinhanTransactionRequestDto = portfolioService.transactionHistoryInquiry(userNumber);
        ...
        for(int i=0;i<=day;i++) {
            ...
            for (ShinhanTransactionRequestDto.DataBody.TransactionHistory transactionHistory : shinhanTransactionRequestDto.getDataBody().getTransactions()) {
                ...
                if(date.equals(plan.get().getStartDate().plusDays(i)) && Integer.parseInt(transactionHistory.getWithdrawalAmount())>0){
...
//                    String extractedCategory = categoryName.split(">")[0].trim();
                    ...
                    if(category.containsKey(extractedCategory)){
                        category.put(extractedCategory,category.get(extractedCategory)+Integer.parseInt(transactionHistory.getWithdrawalAmount()));
                    }
                    else{ // 기타 상승
                        category.put("기타",category.get("기타")+Integer.parseInt(transactionHistory.getWithdrawalAmount()));
                    }
                    ...
                    if (LocalDate.parse(analyzeRequestDto.getDataBody().getTodayDate(), formatter)
                        ...
                        amountUsed += Integer.parseInt(transactionHistory.getWithdrawalAmount());
                    }
        ...
        AnalyzeResponseDto analyzeResponseDto = AnalyzeResponseDto.builder()
                .dataBody(
                        AnalyzeResponseDto.DataBody.builder()
